...
WEIGHTS_PATH = f"weights/raw/{TYPE}__loss={LOSS}__batch_size={BATCH_SIZE}__epoch={{epoch:02d}}__mse={{mse:.2f}}__mae={{mae:.2f}}__{now:%Y-%m-%dT%H:%M:%S}.h5"
...
def main() -> None:
    ...
    title = f"{TYPE}__loss={LOSS}_batch_size={BATCH_SIZE}__{now:%Y-%m-%dT%H:%M:%S}"
