...
# Не настраиваемые константы
DATASET_DIR = f"datasets/{MODEL_TYPE}/raw_data"
HISTORY_IMAGES_DIR = f"history/{MODEL_TYPE}/"
WEIGHTS_PATH = f"weights/{MODEL_TYPE}/raw/loss={LOSS}__batch_size={BATCH_SIZE}__epoch={{epoch:04d}}__mse={{mse:.2f}}__mae={{mae:.2f}}__{now:%Y-%m-%dT%H:%M:%S}.h5"
...
def main() -> None:
    ...
    title = f"loss={LOSS}__batch_size={BATCH_SIZE}__{now:%Y-%m-%dT%H:%M:%S}"
    ...
    plt.savefig(f"{HISTORY_IMAGES_DIR}/{title}.png")
    plt.show()
